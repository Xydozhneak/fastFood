class Hamburger {
    ...
    getPrice() {
      const totalPrice = this.addOns.reduce((acc, addOn) => acc + addOn.price, this.size.price + this.stuffing.price);
      return totalPrice;
    }
  
    getCalories() {
      const totalCalories = this.addOns.reduce((acc, addOn) => acc + addOn.calories, this.size.calories + this.stuffing.calories);
